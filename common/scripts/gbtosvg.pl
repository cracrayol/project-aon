...
if( $bookXML =~ m{^([-\w\@./]+)$} ) {
    ...
    my $dotPath = "$outPath$FILENAME_SEPARATOR$bookCode.dot";
    print qx{$JAVA -classpath "$XALAN_JAR" org.apache.xalan.xslt.Process -IN "$bookXML" -XSL "$dotXSL" -OUT "$dotPath" -PARAM base-URI "$baseURI" -PARAM language "$rulesHash{'language'}"};

    my $svgPath = $rulesHash{'language'} . $FILENAME_SEPARATOR . 'svg' . $FILENAME_SEPARATOR . $rulesHash{'book-series'};
    &make_path( $svgPath );
    unless( -e $svgPath && -d $svgPath ) {
	    die "Unknown error creating output directory\n";
    }
    $svgPath .= "$FILENAME_SEPARATOR$bookCode.svgz";
    print qx{$DOT -Tsvgz -o $svgPath $dotPath};
}
