...
<xsl:strip-space elements="gamebook meta section data ol ul dl li dd footnotes footnote illustration instance table tr th td" />
...
<xsl:param name="usesouvenirfont"><xsl:text>no</xsl:text></xsl:param>
<xsl:param name="papersize"><xsl:text>a4paper</xsl:text></xsl:param>
<xsl:param name="papersize_de"><xsl:text>a5paper</xsl:text></xsl:param>
<xsl:param name="papersize_es"><xsl:text>a4paper</xsl:text></xsl:param>
<xsl:param name="papersize_us"><xsl:text>letterpaper</xsl:text></xsl:param>
...
\ifpdf
    \documentclass[pdftex,a4paper,fontsize=12pt,</xsl:text><xsl:value-of select="$pagelayout" /><xsl:text>,BCOR10mm,DIV=calc,headinclude,cleardoublepage=empty]{scrbook}
\else
    \documentclass[a4paper,fontsize=12pt,</xsl:text><xsl:value-of select="$pagelayout" /><xsl:text>,BCOR10mm,DIV=calc,headinclude,cleardoublepage=empty]{scrbook}
\fi
...
% XMP information for PDF file, could actually be done with pdfinfo
    \usepackage{hyperxmp}
    \hypersetup{%
       pdftitle={</xsl:text><xsl:apply-templates select="/gamebook/meta/title[1]" /><xsl:text>},
       pdfauthor={</xsl:text>
...
%% We need a meta-variable for the series in order to include it here:
 ...
 \else
     \usepackage[colorlinks=true,linkcolor=black,bookmarks=false]{hyperref}
 \fi
...
\begin{titlepage}
 ...
 \vspace*{\stretch{0.7}}
 \begin{center}
  \Huge{\textcolor{titlecolor}{\textbf{</xsl:text><xsl:apply-templates select="/gamebook/meta/title[1]" /><xsl:text>}}} \vspace{-0.8em}\\
...
<xsl:template match="id( 'map' )">
 ...
 <xsl:value-of select="$newparagraph" />
  ...
  <xsl:for-each select="data/illustration[contains( $use-illustrators, concat( ':', meta/creator, ':' ) ) ]">
   ...
   <xsl:value-of select="$newline" />
   <xsl:text>\addcontentsline{toc}{section}{\protect\numberline{}{</xsl:text>
    <xsl:apply-templates select="$map-title" />
   <xsl:text>}}</xsl:text>
   <xsl:value-of select="$newparagraph" />
